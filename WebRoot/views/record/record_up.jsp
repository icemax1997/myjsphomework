...
<html>
  ...
  List<String[]> list = du.getPageData(table,pageSize, currentPage);
   %>
	...
	<div class="page">
          <div class="myreach">
           <a href="views/record/record_up.jsp?currentPage=<%=currentPage+1 %>&table=${param.table}&power=${param.power}" id="fl-r">下一页</a>
              <a href="views/record/record_up.jsp?currentPage=<%=currentPage-1%>&table=${param.table}&power=${param.power}" id="fl-r">上一页</a>
          </div>
          <div class="decoration">当前是第<%=currentPage %>页</div>
        </div>
  </div>
</div>
</div>
